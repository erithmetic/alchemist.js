...
Alchemist.convert = function(amount, currentUnit, newUnit) {
  var prefix = Alchemist.parsePrefix(currentUnit),
      types = Alchemist.conversions[prefix.unit];
  if(Alchemist.conversionTable[types[0]][newUnit] instanceof Array)
    return Alchemist.conversionTable[types[0]][currentUnit][1].call(this, amount);
  else
    return (Alchemist.base(amount, currentUnit, types[0]) / (prefix.exponent * Alchemist.conversionTable[types[0]][newUnit]));
};
...
Alchemist.conversionTable = {
  ...
  "density": {
    ...
    "brix"    : [function(d) { -261.3 / (d - 261.3) }, function(d) { 261.3 - (261.3 / d) }],
    "plato"   : [function(d) { -260.0 / (d - 260.0) }, function(d) { 260.0 - (260.0 / d) }],
    "baume"   : [function(d) { -145.0 / (d - 145.0) }, function(d) { 145.0 - (145.0 / d) }]
  },
  "distance": {
 ...
 "inductance": {
  ...
  "temperature": {
    ...
    "celsius": [function(t) { t + 273.15 }, function(t) { t - 273.15 }], "centrigrade": [function(t) { t + 273.15 }, function(t) { t - 273.15 }],
    "degree_celsius": [function(t) { t + 273.15 }, function(t) { t - 273.15 }], "degree_centrigrade": [function(t) { t + 273.15 }, function(t) { t - 273.15 }],
    "degrees_celsius": [function(t) { t + 273.15 }, function(t) { t - 273.15 }], "degrees_centrigrade": [function(t) { t + 273.15 }, function(t) { t - 273.15 }],
    "fahrenheit": [function(t) { (t + 459.67) * (5.0/9.0) }, function(t) { t * (9.0/5.0) - 459.67 }],
    "degree_fahrenheit": [function(t) { (t + 459.67) * (5.0/9.0) }, function(t) { t * (9.0/5.0) - 459.67 }],
    "degrees_fahrenheit": [function(t) { (t + 459.67) * (5.0/9.0) }, function(t) { t * (9.0/5.0) - 459.67 }],
...
//class CompoundNumericConversion
  ...
  //def consolidate
    //@numerators.each_with_index do |numerator, n|
      //@denominators.each_with_index do |denominator, d|
        ...
        //if (Alchemist.conversions[numerator.unit_name] & Alchemist.conversions[denominator.unit_name]).length > 0
  ...
  //def method_missing(method, *attrs, &block)
    //if Alchemist.conversions[method]
...
Alchemist.base = function(value, currentType, newType) {
  if(Alchemist.conversionTable[newType][currentType] instanceof Array) {
    return Alchemist.conversion_table[newType][currentType][0].call(this, value)
  } else {
    return value * Alchemist.conversionTable[newType][currentType];
  }
};
